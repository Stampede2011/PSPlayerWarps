...
public class Icon implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        if (args.getOne(Text.of("warp")).isPresent()) {
            ...
            if (src instanceof Player) {
                ...
                if (warp.owner.equals(player.getUniqueId()) || player.hasPermission("psplayerwarps.command.icon.bypass")) {
                    try {
                        InventoryTask.openIcons(player, warp, warpName);
            ...
            src.sendMessage(Utilities.getMessage(plugin.config.get().messages.ERROR_COMMAND_SYNTAX.replace("%usage%", "/pwarp icon <warp>"), true));
        }
    ...
    public static CommandSpec build() {
        ...
        return CommandSpec.builder()
                .permission("psplayerwarps.command.icon.base")
